...
def find_room(parti, room, room_file_input, parti_file_input, group_choice=None, find_room_name=None):
    ...
    if group_choice is not None:
        ...
        for participant in all_participant:
            ...
            same_room = []
            found_room = []
            for find_same_room in room_group:
                if find_same_room[0] == participant[group_choice]:

                    if find_same_room in same_room: #kolla att rummet inte redan finns med i listan
                        continue
                        
                        
                    else:
                        same_room.append(find_same_room)

            for find_all_rooms in all_rooms:
                for sroom in same_room:
                    if find_all_rooms[find_room_name] == sroom[1]:
                        found_room.append(find_all_rooms)

           
            try:
            
                random_room = secure_random.choice(found_room)
                if random_room in all_rooms:
                ...
                try:
                    ...
                    add_output(output_parti=participant, output_exception="Missing Room")
